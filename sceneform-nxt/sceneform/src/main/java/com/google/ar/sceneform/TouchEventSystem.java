...
public class TouchEventSystem {
  ...
  @Nullable private Scene.OnTouchAllListener onTouchAllListener;
  ...
  @Nullable private Scene.OnTouchAllListener handlingTouchAllListener = null;
   ...
   * by the nodes.
   *
   * @param onTouchAllListener the touch listener to attach
   */
  public void setOnTouchAllListener(@Nullable Scene.OnTouchAllListener onTouchAllListener) {
    this.onTouchAllListener = onTouchAllListener;
  }
  ...
  public void onTouchAllEvent(ArrayList<HitTestResult> hitTestResultList, MotionEvent motionEvent) {
    tryDispatchToSceneTouchAllListener(hitTestResultList,motionEvent);
  }
  ...
  private boolean tryDispatchToSceneTouchAllListener(
          ArrayList<HitTestResult> hitTestResultList, MotionEvent motionEvent) {
    // This is a new gesture, give the touch listener a chance to capture the input.
    if (motionEvent.getActionMasked() == MotionEvent.ACTION_DOWN) {
      // If the listener handles the gesture, then the event is never propagated to the nodes.
      if (onTouchAllListener != null && onTouchAllListener.onSceneTouchAll(hitTestResultList, motionEvent)) {
        // The touch listener is handling the gesture, return early.
        handlingTouchAllListener = onTouchAllListener;
        return true;
      }
    } else if (handlingTouchAllListener != null) {
      handlingTouchAllListener.onSceneTouchAll(hitTestResultList, motionEvent);
      return true;
    }
