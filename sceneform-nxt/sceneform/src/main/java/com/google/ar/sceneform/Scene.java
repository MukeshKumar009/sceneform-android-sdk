...
public class Scene extends NodeParent {
    ...
    /** Custom touch listener for all the node in layers one behind another.
     * This will dispatch the call back for every node and for the touch intersection of ray cast.
     *
     * Interface definition for a callback to be invoked when a touch event is dispatched to a scene.
     * The callback will be invoked after the touch event is dispatched to the nodes in the scene if
     * no node consumed the event.
     */
    public interface OnTouchAllListener {
        /**
         * Called when a touch event is dispatched to a scene. The callback will be invoked after the
         * touch event is dispatched to the nodes in the scene if no node consumed the event. This is
         * called even if the touch is not over a node, in which case {@link HitTestResult#getNode()}
         * will be null.
         *
         * @param hitTestResultList represents the node list that was touched
         * @param motionEvent   the motion event
         * @return true if the listener has consumed the event
         * @see Scene#setOnTouchAllListener(OnTouchAllListener)
         */
        boolean onSceneTouchAll(ArrayList<HitTestResult> hitTestResultList, MotionEvent motionEvent);
...
//        camera.destroy();
     ...
     */
    public void setOnTouchAllListener(@Nullable OnTouchAllListener onTouchAllListener) {
        touchEventSystem.setOnTouchAllListener(onTouchAllListener);
    }
    ...
    void onTouchEventAll(MotionEvent motionEvent) {
        Preconditions.checkNotNull(motionEvent, "Parameter \"motionEvent\" was null.");

        ArrayList<HitTestResult> hitTestResultList = hitTestAll(motionEvent);
        touchEventSystem.onTouchAllEvent(hitTestResultList,motionEvent);
    }
