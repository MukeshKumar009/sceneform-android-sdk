...
public class VideoNode extends Node {
    ...
    private static final String MATERIAL_PARAMETER_ALPHA = "alpha";
     ...
     * @param listener  Loading listener
     */
    public VideoNode(Context context, MediaPlayer player, @Nullable Color chromaKeyColor, float alpha,
                     @Nullable ExternalTexture texture, @Nullable float cornerRadius, @Nullable Listener listener) {
        this.player = player;
        this.texture = texture != null ? texture : new ExternalTexture();
        this.chromaKeyColor = chromaKeyColor;
        this.alpha = alpha;
    ...
    private void init(Context context) {
        ...
        Material.builder()
                ...
                .thenAccept(material -> {
                    ...
                    material.setFloat(MATERIAL_PARAMETER_ALPHA, alpha);
                    if (chromaKeyColor != null) {
                        material.setFloat4(MATERIAL_PARAMETER_CHROMA_KEY_COLOR, chromaKeyColor);
                    }
                   // final Renderable renderable = createModel(player, material); //default sceneform plane
                    final Renderable renderable = createModelRounded(player, material); //custom rounded corner plane
    ...
    public Renderable createModel(MediaPlayer player, Material material) {
        ...
        //Converting px to meter, in sceneform, scale 1 = 1 meter
        float widthM = (width)/1000;
        float heightM = (height)/1000;

        return makePlane(x, y, material);
    }

    public Renderable createModelRounded(MediaPlayer player, Material material) {
        final int width = player.getVideoWidth();
        final int height = player.getVideoHeight();
        final float x;
        final float y;
        ...
        //Converting real world scale in sceneform, scale 1 = 1 meter
        float widthW = x/1000;
        float heightW = y/1000;

        return makeRoundedPlane(x, y, cornerRadius, material);
    }
    ...
    public Renderable makeRoundedPlane(float width, float height, float cornerRadius, Material material) {
        return PlaneFactory.makeRoundedPlane(
                new Vector3(width, height, 0.0f),
                new Vector3(0.0f, 0.0f, 0.0f), cornerRadius, material);
    }
